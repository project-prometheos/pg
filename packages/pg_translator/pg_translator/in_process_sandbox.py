...
class InProcessSandbox:
                ...
                for blank in answer_blanks:
                    if blank.evaluator_expr:
                        try:
                            ...
                            evaluator = eval(blank.evaluator_expr, globals_context, context)
                            # Register with ANS()
    ...
    def _load_pg_basic_macros_stubs(self) -> None:
        ...
        def ans_rule(width=20):
            env = self.namespace.get('get_environment', lambda: self._stub_env)()
        ...
        try:
            ...
            class StubEvaluator:
                ...
                def evaluate(self, student_answer):
                    ...
                    try:
                        is_correct = float(student_answer) == float(self.correct_answer)
                        return AnswerResult(
    ...
    def execute(self, code: str, seed: int, context: Context | None = None) -> ExecutionResult:
        ...
        try:
            if hasattr(self, '_pg_core'):
                ...
                pg_env = self._pg_environment if hasattr(self, '_pg_environment') else None
        except:
            pg_env = None
        ...
        if pg_env:
            ...
            solution_text = ''.join(getattr(pg_env, 'solution_array', [])) if hasattr(pg_env, 'solution_array') else None
            hint_text = ''.join(getattr(pg_env, 'hint_array', [])) if hasattr(pg_env, 'hint_array') else None
        else:
            output_text = ""
            answers = {}
            solution_text = None
