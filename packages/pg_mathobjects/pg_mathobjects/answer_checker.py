...
class FormulaAnswerChecker(AnswerChecker):
    ...
    def check(self, student_answer: str) -> Dict[str, Any]:
        ...
        try:
            student_formula = Formula(student_answer, self.correct_value.context)
        ...
        import sympy as sp
        correct_vars = sorted([str(s) for s in self.correct_value._tree.free_symbols])
        student_vars = sorted([str(s) for s in student_formula._tree.free_symbols])
