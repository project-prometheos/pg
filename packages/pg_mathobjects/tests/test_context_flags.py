...
import pytest
from pg_mathobjects import Context, Formula, Real
...
class TestToleranceFlags:
    ...
    def test_relative_tolerance_default(self):
        ...
        ctx = Context('Numeric')
        r1 = Real(1.0, ctx)
        r2 = Real(1.0005, ctx)  # Within 0.001 relative tolerance
        r3 = Real(1.002, ctx)   # Outside 0.001 relative tolerance
    ...
    def test_tolerance_in_comparison(self):
        ...
        r1 = Real(100.0, ctx)
        r2 = Real(100.005, ctx)  # 0.005% difference, within 0.0001 relative
    ...
    def test_tolerance_in_formula_eval(self):
        ...
        assert result == Real(9.0, ctx)
        assert result == Real(9.005, ctx)  # Within 1% relative
...
class TestReduceFlags:
    ...
    def test_reduce_constants_affects_simplification(self):
        ...
        assert '5' in str(f1) or f1.eval().value == 5
