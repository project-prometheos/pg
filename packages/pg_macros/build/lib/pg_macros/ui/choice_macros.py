...
class MultipleChoice:
    ...
    def print_a(self) -> str:
        ...
        if self.extra_answers:
            pivot = len(all_choices) - len(self.extra_answers)
            if pivot > 0:
                prefix = all_choices[:pivot]
                suffix = all_choices[pivot:]
                rng.shuffle(prefix)
                all_choices = prefix + suffix
        else:
