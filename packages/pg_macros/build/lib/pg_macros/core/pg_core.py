...
_fallback_answer_queue: List[Any] = []
...
def TEXT(*args: Any) -> str:
    """
    Append text to problem output while returning the concatenated string.
...
def HEADER_TEXT(*args: Any) -> str:
    """
    Append text to HTML header and return the emitted string."""
    text = "".join(str(arg) for arg in args)
    try:
        env = get_environment()
    except RuntimeError:
...
def POST_HEADER_TEXT(*args: Any) -> str:
    """
    Append text after header (DEPRECATED) and return the emitted string."""
    text = "".join(str(arg) for arg in args)
    try:
        env = get_environment()
    except RuntimeError:
...
def ANS(*evaluators: Any) -> None:
    ...
    try:
        ...
        _fallback_answer_queue.extend(evaluators)
        return
...
def NAMED_ANS(name: str, evaluator: Any) -> None:
    ...
    try:
        ...
        _fallback_answer_queue.append((name, evaluator))
        return
