...
def NAMED_ANS_RULE(name: str, width: int = 20, answer_value: str = "") -> str:
    ...
    if mode == "TeX":
        ...
        # HTML mode
        escaped_value = answer_value.replace('"', '&quot;').replace('<', '&lt;').replace('>', '&gt;')
        return (
            f'<input type="text" '
            f'name="{name}" '
            f'id="{name}" '
...
def NAMED_ANS_BOX(name: str, rows: int = 5, cols: int = 80, answer_value: str = "") -> str:
    ...
    if mode == "TeX":
        ...
        lines = ["\\underline{\\phantom{" + "x" * cols + "}}" for _ in range(rows)]
        return "\n\n".join(lines)
    elif mode == "PTX":
        # PreTeXt format
...
def NAMED_ANS_RADIO_BUTTONS(name: str, *options: str) -> list[str]:
    ...
    for i, option in enumerate(options):
        ...
        if mode == "TeX":
            ...
            # PreTeXt format
            buttons.append(f'<choice correct="{"yes" if checked else "no"}">{option}</choice>')
        else:
            # HTML format
            buttons.append(
                f'<label>'
