...
class FormulaAnswerChecker(AnswerChecker):
    ...
    def check(self, student_answer: str) -> Dict[str, Any]:
        ...
        try:
            ...
            student_formula = Formula(student_answer, self.correct_value.variables, self.correct_value.context)
        ...
        correct_vars = sorted([str(s) for s in self.correct_value._sympy_expr.free_symbols])
        student_vars = sorted([str(s) for s in student_formula._sympy_expr.free_symbols])
...
class RealAnswerChecker(AnswerChecker):
    ...
    def __init__(self, correct_value, **options):
        ...
        self.tolerance = options.get('tolerance', tolerance_from_context if tolerance_from_context is not None else 0.001)
        
        tol_type_from_context = correct_value.context.flags.get('tolType')
        self.tol_type = options.get('tolType', tol_type_from_context if tol_type_from_context is not None else 'relative')
