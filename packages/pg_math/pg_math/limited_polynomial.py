...
class PolynomialValidator:
    ...
    def validate(self, formula: Formula) -> Tuple[bool, str | None]:
        ...
        if self.strict:
            is_valid, error = self._check_strict_coefficients(formula._sympy_expr, variables)
            if not is_valid:
                return False, error
        
        # Check single powers if required
        if self.single_powers:
            is_valid, error = self._check_single_powers(formula._sympy_expr, variables)
            if not is_valid:
                return False, error
    ...
    def _check_single_powers(self, expr: sp.Expr, variables: set) -> Tuple[bool, str | None]:
        ...
        for term in terms:
            ...
            for var in variables:
                ...
                if power > 0:
                    var_name = str(var)  # Convert Symbol to string for dict key
