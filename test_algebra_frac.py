...
try:
    r = t.translate('tutorial/sample-problems/Algebra/AlgebraicFractionAnswer.pg', seed=1234)

    print(f'\nStatement HTML: {len(r.statement_html) if r.statement_html else 0} chars')
    print(f'Answer blanks: {len(r.answer_blanks)}')
    print(f'Solution HTML: {len(r.solution_html) if r.solution_html else 0} chars')
