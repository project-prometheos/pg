...
def render_database_problem(
    ...
    # Render using PG translator (full PG compatibility)
    translator = get_pg_translator_service()
    rendered = translator.render_problem(problem['pg_source'], seed=seed)
...
def check_database_problem_answers(
    ...
    # Check answers using PG translator (full PG compatibility)
    translator = get_pg_translator_service()
    results = translator.check_answers(
